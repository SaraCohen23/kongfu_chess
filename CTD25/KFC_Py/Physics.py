...
class MovePhysics(BasePhysics):
    ...
    def update(self, now_ms: int):
        ...
        current_cell = self.board.m_to_cell(self._curr_pos_m)
...
class KnightJumpPhysics(MovePhysics):
    """Special physics for Knight pieces that can jump over other pieces.
    
    Knights move visually along their path like normal pieces, but they don't
    collide with pieces along the way - they only interact with the destination.
    """
    
    def __init__(self, board: Board, param: float = 1.0):
        super().__init__(board, param)
        # Knights don't need clear path - they can jump over pieces
        self.do_i_need_clear_path = False
        
    def is_need_clear_path(self) -> bool:
        return False
        
    def reset(self, cmd: Command):
        """Knight moves visually like normal pieces but can pass through others."""
        
        # Use the normal MovePhysics movement (visual path)
        super().reset(cmd)
        
        
    def can_be_captured(self) -> bool:
        """Knights can be captured while moving (unlike JumpPhysics)."""
        return True
        
    def is_movement_blocker(self) -> bool:
        """Knights don't block other pieces' movement."""
        return False
        
    def is_at_destination(self) -> bool:
        """Check if knight has reached its final destination."""
        if not hasattr(self, '_end_cell'):
            return True
        current_cell = self.board.m_to_cell(self._curr_pos_m)
        at_dest = current_cell == self._end_cell
        return at_dest
        
    def can_capture_at_current_position(self) -> bool:
        """Knights can only capture when they reach their destination."""
        result = self.is_at_destination()
        return result
