...
class Board:
    ...
    board_offset_x: int = 0  #(X)
    board_offset_y: int = 0  #(Y)

    # convenience, not required by dataclass
    def clone(self) -> "Board":
        return Board(cell_H_pix=self.cell_H_pix, cell_W_pix=self.cell_W_pix,
                     W_cells=self.W_cells, H_cells=self.H_cells,
                     img=self.img.copy(), 
                     cell_H_m=self.cell_H_m, cell_W_m=self.cell_W_m,
    ...
    def m_to_pix(self, pos_m: tuple[float, float]) -> tuple[int, int]:
        """Convert *(x, y)* in metres to pixel coordinates with board offset."""
        x_m, y_m = pos_m
        x_px = int(round(x_m / self.cell_W_m * self.cell_W_pix)) + self.board_offset_x
        y_px = int(round(y_m / self.cell_H_m * self.cell_H_pix)) + self.board_offset_y
        return x_px, y_px
