...
def test_gameplay_pawn_move_and_capture():
    ...
    time.sleep(0.5)
    game.run(num_iterations=200, is_with_graphics=False)
    assert pw.current_cell() == (4, 0)
    assert pb.current_cell() == (3, 1)
    time.sleep(0.5)
    game._run_game_loop(num_iterations=200, is_with_graphics=False)
    game.user_input_queue.put(Command(game.game_time_ms(), pw.id, "move", [(4, 0), (3, 1)]))
    time.sleep(0.5)
    game._run_game_loop(num_iterations=300, is_with_graphics=False)
...
def test_pawn_double_step_only_first_move():
    ...
    game._run_game_loop(num_iterations=250, is_with_graphics=False)
    time.sleep(0.1)
