    ...
    def on_command(self, cmd: Command, cell2piece: Dict[Tuple[int, int], List[Piece]], my_color: str = "X"):
        ...
        if cmd.type == "move":
            if self.moves is None or len(cmd.params) < 2:
                raise ValueError(f"Invalid move command: params={cmd.params}")
            ...
            if src_cell != current_cell:
                raise ValueError(f"source cell {src_cell} is not the current cell {current_cell}")

            is_valid = self.moves.is_valid(src_cell, dst_cell, cell2piece, self.physics.is_need_clear_path(), my_color)
