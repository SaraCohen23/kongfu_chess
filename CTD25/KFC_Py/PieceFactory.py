...
class PieceFactory:
    ...
    # ──────────────────────────────────────────────────────────────
    def _build_state_machine(self, piece_dir: pathlib.Path, piece_type: str = None) -> State:
        ...
        for state_dir in (piece_dir / "states").iterdir():
            ...
            physics = self.physics_factory.create((0, 0), name, physics_cfg, piece_type)
    ...
    def create_piece(self, p_type: str, cell: Tuple[int, int]) -> Piece:
        ...
        state = self._build_state_machine(p_dir, p_type)
