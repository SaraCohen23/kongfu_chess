import threading, logging
try:
    import keyboard  # pip install keyboard
except ImportError as e:
    raise
from Command import Command

logger = logging.getLogger(__name__)


class KeyboardProcessor:
    """
    Maintains a cursor on an R×C grid and maps raw key names
    into logical actions via a user‑supplied keymap.
    """

    def __init__(self, rows: int, cols: int, keymap: dict[str, str]):
        self.rows = rows
        self.cols = cols
        self.keymap = keymap
        self._cursor = [0, 0]  # [row, col]
        self._lock = threading.Lock()

    def process_key(self, event):
        # Only care about key‑down events
        if event.event_type != "down":
            return None

        key = event.name
        action = self.keymap.get(key)
        logger.debug("Key '%s' → action '%s'", key, action)

        if action in ("up", "down", "left", "right"):
            with self._lock:
                r, c = self._cursor
                old_pos = (r, c)
                if action == "up":
                    r = max(0, r - 1)
                elif action == "down":
                    r = min(self.rows - 1, r + 1)
                elif action == "left":
                    c = max(0, c - 1)
                elif action == "right":
                    c = min(self.cols - 1, c + 1)
                self._cursor = [r, c]
                logger.debug("Cursor moved to (%s,%s)", r, c)

        return action

    def get_cursor(self) -> tuple[int, int]:
        with self._lock:
            return tuple(self._cursor)


class KeyboardProducer(threading.Thread):
    """
    Runs in its own daemon thread; hooks into the `keyboard` lib,
    polls events, translates them via the KeyboardProcessor,
    and turns `select`/`jump` into Command objects on the Game queue.
    Each producer is tied to a player number (1 or 2).
    """

    def __init__(self, game, queue, processor: KeyboardProcessor, player: int):
        super().__init__(daemon=True)
        self.game = game
        self.queue = queue
        self.proc = processor
        self.player = player
        self.selected_id = None
        self.selected_cell = None

    def _is_my_key(self, key: str) -> bool:
        """Check if this key belongs to this player."""
        if self.player == 1:
            # Player 1 keys: arrow keys, enter, +, esc
            player1_keys = {"up", "down", "left", "right", "enter", "+", "esc"}
            return key in player1_keys
        elif self.player == 2:
            # Player 2 keys: WASD, f, g, esc (both English and Hebrew)
            player2_keys = {"w", "s", "a", "d", "f", "g", "esc", 
                           "ץ", "ד", "ש", "ג", "כ", "ע"}  # Hebrew equivalents
            return key in player2_keys
        return False

    def run(self):
        # Install our hook; it stays active until we call keyboard.unhook_all()
        try:
            keyboard.hook(self._on_event)
            keyboard.wait()
        except Exception as e:
            pass

    def _find_piece_at(self, cell):
        for p in self.game.pieces:
            if p.current_cell() == cell:
                return p
        return None

    def _on_event(self, event):
        # Only process key-down events  
        if event.event_type != "down":
            return
            
        key = event.name
        
        # Check if this key belongs to this player
        if not self._is_my_key(key):
            return  # Ignore keys that don't belong to this player
            
        action = self.proc.process_key(event)
        
        # Handle exit command immediately
        if action == "exit":
            cmd = Command(
                self.game.game_time_ms(),
                "SYSTEM",  # Special system command
                "exit",
                []
            )
            self.queue.put(cmd)
            # Clear selection when exiting
            self.selected_id = None
            self.selected_cell = None
            setattr(self.game, f'selected_piece_{self.player}', None)
            logger.info(f"Player{self.player} pressed ESC - Exit command queued")
            return
        
        # only interpret select/jump for game pieces
        if action not in ("select", "jump"):
            return

        cell = self.proc.get_cursor()
        # read/write the correct selected_id_X on the Game
        if action == "select":
            if self.selected_id is None:
                # first press = pick up the piece under the cursor
                piece = self._find_piece_at(cell)
                if not piece:
                    return

                # Check if this player can control this piece
                if not self._can_control_piece(piece):
                    return

                self.selected_id = piece.id
                self.selected_cell = cell
                # Update the game's selected piece for visual indication
                setattr(self.game, f'selected_piece_{self.player}', piece.id)
                return

            elif cell == self.selected_cell:  # selected same place
                self.selected_id = None
                # Clear the visual selection
                setattr(self.game, f'selected_piece_{self.player}', None)
                return

            else:
                cmd = Command(
                    self.game.game_time_ms(),
                    self.selected_id,
                    "move",
                    [self.selected_cell, cell]
                )
                self.queue.put(cmd)
                logger.info(f"Player{self.player} queued {cmd}")
                self.selected_id = None
                self.selected_cell = None
                # Clear the visual selection after move
                setattr(self.game, f'selected_piece_{self.player}', None)

        elif action == "jump":
            if self.selected_id is None:
                # first press = pick up the piece under the cursor
                piece = self._find_piece_at(cell)
                if not piece:
                    return

                # Check if this player can control this piece
                if not self._can_control_piece(piece):
                    return

                self.selected_id = piece.id
                self.selected_cell = cell
                # Update the game's selected piece for visual indication
                setattr(self.game, f'selected_piece_{self.player}', piece.id)
                return

            else:
                # For JUMP, allow jumping to the same location (useful for in-place actions)
                cmd = Command(
                    self.game.game_time_ms(),
                    self.selected_id,
                    "jump",
                    [self.selected_cell, cell]
                )
                self.queue.put(cmd)
                logger.info(f"Player{self.player} queued {cmd}")
                self.selected_id = None
                self.selected_cell = None
                # Clear the visual selection after jump
                setattr(self.game, f'selected_piece_{self.player}', None)

    def _can_control_piece(self, piece) -> bool:
        """Check if this player can control the given piece."""
        piece_color = piece.id[1]  # 'W' for White, 'B' for Black
        
        if self.player == 1:
            # Player 1 (Green cursor) controls White pieces
            return piece_color == 'W'
        elif self.player == 2:
            # Player 2 (Red cursor) controls Black pieces  
            return piece_color == 'B'
        
        return False

    def stop(self):
        keyboard.unhook_all()